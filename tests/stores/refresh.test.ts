...
describe('useRefreshStore', () => {
  ...
  it('should fetch tasks and update data', async () => {
    ...
    vi.spyOn(Project, 'readProjectProjectProjectIdGet').mockResolvedValue(mockTasks)

    const data = await store.fetchTasks(123)

    expect(Project.readProjectProjectProjectIdGet).toHaveBeenCalledWith({path: {project_id:123}})
  ...
  it('should fetch annotations and update data', async () => {
    ...
    vi.spyOn(Task, 'readTaskTaskTaskIdGet').mockResolvedValue(mockAnnotations)

    const data = await store.fetchAnnotations(456)

    expect(Task.readTaskTaskTaskIdGet).toHaveBeenCalledWith({
      path: { task_id: 456 },
    });
...
it('should handle onResponse and update project_number', async () => {
  ...
  const setProjectNumberSpy = vi.spyOn(refreshStore, 'setProjectNumber')

  vi.spyOn(client, 'get').mockImplementation(async ({ onResponse }) => {
    await onResponse?.({
      response: {
        headers: { get: (key: string) => key === 'x-total-count' ? '123' : null }
      }
    })
    return [{ id: 1 }]
  })
