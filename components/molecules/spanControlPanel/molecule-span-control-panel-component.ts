...
export default defineNuxtComponent({
  ...
  setup(props, {emit}){
    ...
    const mainPluginName = computed(()=>pluginList.value.find(plugin=>plugin.id == mainPluginId.value)?.name )
    const mainGroupPluginName = computed(()=>pluginList.value.find(plugin=>plugin.id == mainGroupPluginId.value)?.name )
