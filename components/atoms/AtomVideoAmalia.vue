<template>
  ...
  <div class="w-full flex justify-center rounded-lg ">
    <Button icon="pi pi-history" severity="secondary" :disabled="!showRollback" rounded
...
async function fetchVideoStream(url) {
  ...
  mediaType.value = parsedUrl.searchParams.get("typemedia");
...
async function fetchRedirectUrl(url: string) {
  const response = await fetch(url, {headers:media_params?.headers??new Headers()}).then((resp) => {
    return resp.text()
  });
  return response;
}
const playerParam = async () => {
  if (media_params?.thumbnail_base_url) {
    dynamicTumbnails.value = await fetchRedirectUrl(media_params?.thumbnail_base_url)
  }
  if (media_params?.download_base_url) {
    downloadUrl.value = await fetchRedirectUrl(media_params?.download_base_url)
  }
  if (media_params?.waveform_base_url) {
    waveformUrl.value = await fetchRedirectUrl(media_params?.waveform_base_url)
  }
}
