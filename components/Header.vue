<template>
  ...
  <Dialog
    ...
    <div class=" pt-5 flex flex-col gap-4">
      ...
      <Dropdown v-model="selectedType" :options="['🏷️Segmentation','✒️Transcription','Identification des visages']"
    ...
    <div class="pt-8">
      <Toast/>
      <FileUpload
        accept="application/json" :show-upload-button=false :show-cancel-button="false" :max-file-size=maxFileSize
        ...
        <template #header={chooseCallback}>
          <div class="w-full" @click="chooseCallback()"/>
        </template>

        <template #content="{uploadedFiles, files, removeUploadedFileCallback }">
          <div class="flex flex-col gap-2">
            <div v-for="(file,index) in uploadedFiles" :key="index" class="grid grid-cols-8 gap-2 px-1 items-center">
              ...
              <p v-else class="text-slate-400 text-xs text-nowrap col-span-2  ">{{ Math.round(file.size / 1024) }}
                KB</p>
              <Button
