<template>
  <div class=" text-title gap-3 grid  " :style="[ thumbnailUrl != '' ? 'grid-template-columns: repeat(auto-fill,minmax(210px, 1fr)' : 'grid-template-columns: repeat(auto-fill,minmax(120px, 1fr)']"  >
    <div v-for="(timecode,index) in getHistory" :key="index" data-test="timecode-item" class="flex overflow-clip h-[74px] justify-around rounded-md p-sm group hover:text-[#0b7698] hover:bg-secondary cursor-pointer font-bold gap-5 items-center transition-all duration-300  " @click="videoPlayer.consumeTimecode(index)">
